...
export default function ScheduleCell({ day: _day, hour, blocks = [], isBlocked = false, onClick }: ScheduleCellProps) {
  ...
  if (block && isFirstHour && activeBlock) {
    return (
      <div
        ...
        style={{ 
          backgroundColor: activeBlock.color ? `${activeBlock.color}20` : undefined, 
  ...
  if (block && isFirstHour && blockedBlock && !hasActiveBlocks) {
    return (
      <div
        className="schedule-cell blocked"
        style={{ 
          backgroundColor: blockedBlock.color ? `${blockedBlock.color}20` : undefined, 
          borderLeftColor: blockedBlock.color || undefined 
        }}
        title="Bloqueado - tienes clase en este horario"
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="lock-icon">
  ...
  if (block && blockedBlock && !hasActiveBlocks) {
    return (
      <div
        className="schedule-cell blocked"
        style={{ 
